...
function sendMail(email, userId, token) {
    ...
    mail.send(email, 'Берем Все: Регистрация', null, '<div>От вашего email поступил запрос на регистрацию в сервисе "Берем Все"</div>\
...
async function addUser(ctx, next, urlParts, query, db, koauth) {
    ...
    try {
        ...
        ctx.request.body.params.data.level = 0xffffffff;
